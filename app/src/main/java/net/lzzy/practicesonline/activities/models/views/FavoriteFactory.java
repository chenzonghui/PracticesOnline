...
public class FavoriteFactory  {
    ...
    private Favorite getByQuestion(String questionId) {
        try {
            List<Favorite> favorites = repository.getByKeyword(questionId, new String[]{
                    Favorite.COL_QUESTINON_IP}, true);
            if (favorites.size() > 0) {
        ...
        public boolean isQueetionstionSarred(String questionId){
            try {
                List<Favorite> favories= repository.getByKeyword(questionId,
                        new String[]{Favorite.COL_QUESTINON_IP},true);
                return  favories.size()>0;
            } catch (IllegalAccessException |InstantiationException e) {
                 e.printStackTrace();
                 return false;
            }
        ...
        if (favorite == null){
            ...
            favorite.setQuestionld(questionId);
